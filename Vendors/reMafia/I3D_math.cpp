...
{
	float len = sqrtf((float)vec.Magnitude2());
	if (len != 0.0f)
	{
		this->x = vec.x / len;
		this->y = vec.y / len;
...
double S_vector::CosAngleTo(S_vector const& vec)
{
	...
	return sqrtf((val + 1.0f) * 0.5f);
}

double S_vector::AngleTo(S_vector const& v)
{
	double f = Magnitude() * v.Magnitude();
	if (f < MRG_ZERO) return 0.0f;
	double acos_val = Dot(v) / f;
	return acos(Min<double>(Max<double>(acos_val, -1.0f), 1.0f));
}
...
void __stdcall S_quat::Normalize()
{
	...
	float invSqrt = clamp<float>(1.0f / sqrt(mag2), -1.0f, 1.0f);
	w = w * invSqrt;
	x = x * invSqrt;
	y = y * invSqrt;
	z = z * invSqrt;
